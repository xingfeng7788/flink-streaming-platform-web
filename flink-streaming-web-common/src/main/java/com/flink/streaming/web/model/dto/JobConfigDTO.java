...
public class JobConfigDTO implements Serializable {
  ...
  public List<String> getExtJarPathUrl(String downloadUrlPrefix) {
    if (extJarPath == null) {
    ...
    for (String url : urls) {
      ...
      } else if (url.trim().startsWith("s3://")) {
        String fileName = url.trim().substring(url.trim().lastIndexOf('/') + 1);
        urlJarsList.add(String.format("%s/%s", downloadUrlPrefix, fileName));
      } else {
  ...
  public static JobConfig toEntity(JobConfigDTO jobConfigDTO) {
    ...
    jobConfig.setClusterName(jobConfigDTO.getClusterName());
    jobConfig.setClusterMasterIp(jobConfigDTO.getClusterMasterIp());
    jobConfig.setYarnRmPort(jobConfigDTO.getYarnRmPort());
    jobConfig.setFlinkUiPort(jobConfigDTO.getFlinkUiPort());
    jobConfig.setFlinkHaHttpPort(jobConfigDTO.getFlinkHaHttpPort());
  ...
  public static JobConfigDTO toDTO(JobConfig jobConfig) {
    ...
    jobConfigDTO.setClusterName(jobConfig.getClusterName());
    jobConfigDTO.setClusterMasterIp(jobConfig.getClusterMasterIp());
    jobConfigDTO.setYarnRmPort(jobConfig.getYarnRmPort());
    jobConfigDTO.setFlinkUiPort(jobConfig.getFlinkUiPort());
    jobConfigDTO.setFlinkHaHttpPort(jobConfig.getFlinkHaHttpPort());
