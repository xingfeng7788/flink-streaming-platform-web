...
public class JobConfigServiceImpl implements JobConfigService {
  ...
  public Long addJobConfig(JobConfigDTO jobConfigDTO) {
    ...
    this.checkSystemConfig(jobConfigDTO.getClusterId(), jobConfigDTO.getDeployModeEnum());
  ...
  public void updateJobConfigById(JobConfigDTO jobConfigDTO) {
    ...
    if (jobConfigDTO.getDeployModeEnum() == null) {
      this.checkSystemConfig(jobConfigDTO.getClusterId(), DeployModeEnum.valueOf(jobConfig.getDeployMode()));
    } else {
      this.checkSystemConfig(jobConfigDTO.getClusterId(), jobConfigDTO.getDeployModeEnum());
    }
  ...
  public List<JobConfigDTO> findJobConfigByStatus(Integer... status) {
    ...
    // 添加通过ip获取集群信息, 定时调度只运行本集群相关的任务
    ClusterDTO clusterDTO = clusterMapper.selectByIp(ip);

    if (clusterDTO == null) {
      log.warn("当前服务器ip:{} 未找到集群信息", ip);
      return Collections.emptyList();
    }
    log.info("当前服务器ip:{} 找到集群信息:{}", ip, clusterDTO.toString());
    return JobConfigDTO.toListDTO(jobConfigMapper.findJobConfigByStatus(statusList, clusterDTO.getId()));
  }
  ...
  public List<BatchJob> getAllBatchJobs() {
    ...
    // 添加通过ip获取集群信息, 定时调度只运行本集群相关的任务
    ClusterDTO clusterDTO = clusterMapper.selectByIp(ip);
    return jobConfigMapper.getAllBatchJobs(clusterDTO.getId());
  }
   ...
   * @time 22:43
   */
  private void checkSystemConfig(Long clusterId, DeployModeEnum deployModeEnum) {
    ...
    switch (deployModeEnum) {
      ...
      case YARN_APPLICATION:
        String yarnHttpAddress = systemConfigService.getYarnRmHttpAddress(clusterId);
        if (StringUtils.isEmpty(yarnHttpAddress)) {
          tips.append(" yarn_rm_http_address url地址 ");
        }
        break;
      case LOCAL:
        String flinkHttpAddress = systemConfigService.getFlinkHttpAddress(clusterId, deployModeEnum);
        if (StringUtils.isEmpty(flinkHttpAddress)) {
          tips.append(" flink_rest_http_address url地址 ");
        }
        break;
      case STANDALONE:
        String flinkHaHttpAddress = systemConfigService.getFlinkHttpAddress(clusterId, deployModeEnum);
