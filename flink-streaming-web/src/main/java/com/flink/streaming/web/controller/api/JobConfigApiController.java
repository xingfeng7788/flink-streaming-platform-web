...
public class JobConfigApiController extends BaseController {
  ...
  private void postRequestToOtherMachine(String router, Long jobId, HttpServletRequest request) {
    // 从数据库获取作业配置
    JobConfigDTO jobConfigDTO = jobConfigService.getJobConfigById(jobId);
    // 判断连接是否正常 如果不正常任务设置为停止
    String checkUrl = systemConfigService.getFlinkUrl(jobConfigDTO.getClusterId(), jobConfigDTO.getDeployModeEnum());
...
//
      ...
      jobConfig.setJobId("");
      jobConfig.setClusterId(jobConfigDTO.getClusterId());
      jobConfig.setIsOpen(0);
      jobConfigService.updateJobConfigById(jobConfig);
      throw new BizException("router: /api/" + router + "远程调用失败, 请检查集群是否正常, 网络连接失败");
    }
    // 创建请求头映射
    ...
    String url = String.format("http://%s:%s/api/%s", jobConfigDTO.getClusterMasterIp(), customConfig.getWebPort(), router);
    // 使用CloseableHttpClient和HttpPost构造HTTP请求
    try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
      ...
      try (CloseableHttpResponse response = httpClient.execute(httpPost)) {
        ...
        if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
          // 如果是200则认为操作成功，将结果包装在RestResult中返回
          throw new BizException("router: /api/" + router + "远程调用失败");
        }
      }
    } catch (IOException e) {
      // 通信失败
      log.error("HTTP请求调用失败 id={}", jobId, e);
      throw new RuntimeException(e);
    }
  }
  @RequestMapping("/start")
  public RestResult<String> start(Long id, Long savepointId, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("start", id, request);
      } else {
  ...
  @RequestMapping("/stop")
  public RestResult<String> stop(Long id, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("stop", id, request);
      } else {
  ...
  public RestResult<String> close(Long id, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("close", id, request);
      } else {
  ...
  @RequestMapping("/open")
  public RestResult<String> open(Long id, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("open", id, request);
      } else {
  ...
  public RestResult<String> delete(Long id, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("delete", id, request);
      } else {
  ...
  public RestResult<String> savepoint(Long id, HttpServletRequest request) {
    try {
      if (SystemConstants.getIsMasterWeb()) {
        this.postRequestToOtherMachine("savepoint", id, request);
      } else {
  ...
  private RestResult checkUpsertJobConfigParam(UpsertJobConfigParam upsertJobConfigParam) {
    ...
    if (upsertJobConfigParam.getClusterId() == null) {
      return RestResult.error("集群ID不能为空");
    }
    ...
    if (upsertJobConfigParam.getJobType() == null || JobTypeEnum.CDC_STREAMING.getCode() == upsertJobConfigParam.getJobType().intValue()) {
      if (StringUtils.isEmpty(upsertJobConfigParam.getFlinkSql())) {
        return RestResult.error("yaml配置不能为空");
      }
    }
    // sql配置需要校验的参数JobType=null是兼容之前配置
    if (upsertJobConfigParam.getJobType() == null || JobTypeEnum.SQL_STREAMING.getCode() == upsertJobConfigParam.getJobType().intValue()) {
      if (StringUtils.isEmpty(upsertJobConfigParam.getFlinkSql())) {
...
//CHECKSTYLE:ON
  ...
  private void completeJObConfigDTO(JobConfigDTO jobConfigDTO) {
    ...
    domainKey.put(DeployModeEnum.YARN_PER, systemConfigService.getYarnRmHttpAddress(jobConfigDTO.getClusterId()));
    domainKey.put(DeployModeEnum.YARN_APPLICATION, domainKey.get(DeployModeEnum.YARN_PER));
    domainKey.put(DeployModeEnum.LOCAL, systemConfigService.getFlinkAddress(jobConfigDTO.getClusterId(), DeployModeEnum.LOCAL));
    domainKey.put(DeployModeEnum.STANDALONE, systemConfigService.getFlinkAddress(jobConfigDTO.getClusterId(), DeployModeEnum.STANDALONE));
    // 补充FlinkRunUrl字段
    ...
    if (StringUtils.isNotEmpty(domain)) {
      if ((DeployModeEnum.YARN_PER.equals(jobConfigDTO.getDeployModeEnum())
        ...
        jobConfigDTO.setFlinkRunUrl(HttpUtil.buildUrl(domain, FlinkYarnRestUriConstants.getAwsUriOverviewForYarn(jobConfigDTO.getJobId())));
      }
    ...
    String flinkWebUrl = "http://" + jobConfigDTO.getClusterMasterIp() + ":" + jobConfigDTO.getFlinkUiPort() + "/";
    jobConfigDTO.setFlinkWebUrl(HttpUtil.buildUrl(flinkWebUrl, FlinkYarnRestUriConstants.getUriOverviewForYarn(jobConfigDTO.getJobId())));
    // 补充AlarmStrs字段
  ...
  private void completeJObConfigDTO(PageModel<JobConfigDTO> pageModel) {
    ...
    for (JobConfigDTO jobConfigDTO : pageModel) {
      // 补充FlinkRunUrl字段
      String domain = systemConfigService.getFlinkUrl(jobConfigDTO.getClusterId(), jobConfigDTO.getDeployModeEnum());
