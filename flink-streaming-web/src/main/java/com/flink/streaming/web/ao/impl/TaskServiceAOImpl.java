...
public class TaskServiceAOImpl implements TaskServiceAO {
  ...
  public void checkJobStatus() {
    ...
    log.info("状态校验【checkJobStatus】 jobConfigDTOList size={}", jobConfigDTOList.size());
      ...
      switch (jobConfigDTO.getDeployModeEnum()) {
        ...
        case YARN_APPLICATION:
          ...
          try {
            ...
            appId = yarnRestRpcAdapter.getAppIdByYarn(jobConfigDTO.getClusterId(), jobConfigDTO.getJobName(), queueName);
          ...
          if (!StringUtils.isEmpty(appId)) {
            JobInfo jobInfo = yarnRestRpcAdapter.getJobInfoForPerYarnByAppId(jobConfigDTO.getClusterId(), appId);
            if (jobInfo != null && SystemConstants.STATUS_RUNNING.equals(jobInfo.getStatus())) {
              log.warn("执行停止操作 jobYarnInfo={} id={}", jobInfo, appId);
              yarnRestRpcAdapter.cancelJobForYarnByAppId(jobConfigDTO.getClusterId(), appId, jobInfo.getId());
            }
          }
          break;
        default:
  ...
  public void autoSavePoint() {
    ...
    for (JobConfigDTO jobConfigDTO : jobConfigDTOList) {
      ...
      if (JobTypeEnum.SQL_STREAMING.equals(jobConfigDTO.getJobTypeEnum())
          ...
          || JobTypeEnum.CDC_STREAMING.equals(jobConfigDTO.getJobTypeEnum())
      ) {
  ...
  private void checkYarn(JobConfigDTO jobConfigDTO, List<AlarmTypeEnum> alarmTypeEnumList) {
    ...
    //查询任务状态
    log.info("check Yarn job getJobName={} jobId={}", jobConfigDTO.getJobName(), jobConfigDTO.getJobId());
    JobInfo jobInfo = yarnRestRpcAdapter.getJobInfoForPerYarnByAppId(jobConfigDTO.getClusterId(), jobConfigDTO.getJobId());
    if (jobInfo != null && (SystemConstants.STATUS_RUNNING.equals(jobInfo.getStatus())
    ...
    JobConfigDTO jobConfig = JobConfigDTO.bulidStop(jobConfigDTO.getId(), jobConfigDTO.getClusterId());
  ...
  private void checkStandalone(JobConfigDTO jobConfigDTO, List<AlarmTypeEnum> alarmTypeEnumList) {
    ...
    JobStandaloneInfo jobStandaloneInfo = flinkRestRpcAdapter
        .getJobInfoForStandaloneByAppId(jobConfigDTO.getClusterId(), jobConfigDTO.getJobId(), jobConfigDTO.getDeployModeEnum());
