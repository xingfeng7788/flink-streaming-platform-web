      ...
      <el-form ref="queryform" :model="queryform" :inline="true">
        ...
        <el-form-item>
          <el-select v-model="queryform.jobType" placeholder="任务类型" clearable @change="handleQuery()">
            ...
            <el-option label="CDC流任务" value="3" />
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <router-link :to="{name:getRouteTaskName('create','CDC_STREAMING'), params:{flag:'create', context: queryContent(), data:{jobTypeEnum:'CDC_STREAMING'}}}" class="wl-append">
                <el-dropdown-item icon="iconfont my-icon-file-SQL">CDC流任务</el-dropdown-item>
              </router-link>
            </el-dropdown-menu>
          </el-dropdown>
        </el-form-item>
        ...
        <el-table-column prop="id" :show-overflow-tooltip="true" label="ID" min-width="60" width="80" align="center" fixed>
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" :content="getTaskTypeName(scope.row.jobTypeEnum)" placement="right">
              ...
              <i v-if="scope.row.jobTypeEnum==='CDC_STREAMING'" class="iconfont my-icon-file-SQL" style="font-size:16px;" />
            </el-tooltip>
          </template>
        </el-table-column>
<!--        <el-table-column prop="jobName" :show-overflow-tooltip="true" label="任务名称" min-width="100" align="center" fixed />-->
        ...
        <el-table-column prop="jobName" :show-overflow-tooltip="true" label="任务名称" min-width="100" align="center" fixed >
          <template slot-scope="scope">
            <el-link v-if="scope.row.jobId" :href="scope.row.flinkWebUrl" target="_blank" style="color: #1890ff;">{{ scope.row.jobName }}</el-link>
        ...
        <el-table-column prop="clusterName" :show-overflow-tooltip="true" label="集群名称" width="105" align="center" />
        ...
        <el-table-column prop="jobId" :show-overflow-tooltip="true" label="Flink任务Id" min-width="100" align="center">
          <template slot-scope="scope">
            <el-link v-if="scope.row.jobId" :href="scope.row.flinkRunUrl" target="_blank" style="color: #1890ff;" >{{ scope.row.jobId }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="savepoint" label="保存状态(savepoint)" width="135px" fixed="right">
          <template slot-scope="scope">
            <el-link v-if="scope.row.jobTypeEnum==='SQL_STREAMING'|| scope.row.jobTypeEnum === 'JAR' || scope.row.jobTypeEnum === 'CDC_STREAMING'" type="primary" icon="el-icon-sell" @click.native="doRecoverSavePoint(scope.row)">恢复</el-link>
            <el-link v-if="scope.row.jobTypeEnum==='SQL_STREAMING'|| scope.row.jobTypeEnum === 'JAR' || scope.row.jobTypeEnum === 'CDC_STREAMING'" type="success" icon="el-icon-sold-out" @click.native="savePoint(scope.row)">备份</el-link>
          </template>
        </el-table-column>
      ...
      switch (flag) {
        case 'create':
          switch (jobTypeEnum) {
            ...
            case 'CDC_STREAMING': return 'CreateCdcStreamingTask'
        ...
        case 'update':
          switch (jobTypeEnum) {
            ...
            case 'CDC_STREAMING': return 'UpdateCdcStreamingTask'
        ...
        case 'view':
          switch (jobTypeEnum) {
            ...
            case 'CDC_STREAMING': return 'ViewCdcStreamingTask'
